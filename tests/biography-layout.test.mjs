...
test("History uses one flush-left editorial column with shared spacing", async () => {
  ...
  assert.match(styles, /--biography-section-gap:\s*48px;/);
  assert.match(styles, /--biography-label-gap:\s*16px;/);
  ...
  assert.match(styles, /@media screen and \(max-width:\s*599px\)[\s\S]*--biography-section-gap:\s*32px;/);
  ...
  assert.match(styles, /\.biography-clients\s*\{[^}]*grid-column:\s*1 \/ -1;[^}]*margin-top:\s*32px;/s);
  assert.doesNotMatch(styles, /--biography-rule|border-(?:top|bottom):/);
...
test("History has readable type and only two orange editorial accents", async () => {
  ...
  assert.deepEqual(accents, [
    ".biography-editorial .biography-introduction__practice-label",
    '.biography-contact__link[href^="mailto:"]',
  ]);
...
test("History registers stack flush left on phones and clear the fixed menu", async () => {
  ...
  assert.match(styles, /\.biography-experience-section__heading\s*\{[^}]*padding-bottom:\s*24px;/s);
  assert.match(styles, /\.biography-experience\s*\{[^}]*display:\s*grid;[^}]*row-gap:\s*24px;/s);
  assert.match(styles, /\.biography-experience\s*\{[^}]*padding:\s*0;[^}]*list-style:\s*none;/s);
  assert.match(styles, /@media screen and \(max-width:\s*599px\)[\s\S]*\.biography-experience__row\s*\{[^}]*grid-template-columns:\s*minmax\(0, 1fr\);[^}]*row-gap:\s*8px;/);
  assert.match(styles, /@media screen and \(max-width:\s*599px\)[\s\S]*\.biography-capabilities,\s*\.biography-contact\s*\{[^}]*grid-template-columns:\s*minmax\(0, 1fr\);/);
  assert.match(styles, /@media screen and \(max-width:\s*991px\)[\s\S]*\.biography-layout\s*\{[^}]*grid-template-columns:\s*minmax\(0, 1fr\);/);
  assert.match(styles, /padding-bottom:\s*calc\(144px \+ env\(safe-area-inset-bottom, 0px\)\);/);
});
