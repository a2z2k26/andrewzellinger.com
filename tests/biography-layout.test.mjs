...
test("History registers stack flush left on phones and clear the fixed menu", async () => {
  ...
  assert.match(styles, /\.biography-contact\s*\{[^}]*flex-direction:\s*column;[^}]*align-items:\s*flex-start;[^}]*gap:\s*0;/s);
  assert.match(styles, /\.biography-contact__socials\s*\{[^}]*flex-direction:\s*column;[^}]*align-items:\s*flex-start;[^}]*gap:\s*0;[^}]*margin-top:\s*var\(--biography-label-gap\);/s);
