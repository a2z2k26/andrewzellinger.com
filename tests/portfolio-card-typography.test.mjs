...
function assertPortfolioTreatment(source, label) {
  assert.match(source, /html\.works-motion-route\s*\{[^}]*--works-card-gap:\s*24px;/s, `${label} must define the 24px Projects card gap`);
  ...
  assert.match(source, /\.heading-style-h2\.new\s*\{[^}]*font-family:\s*var\(--fonts--family-display\);[^}]*font-size:\s*22px;[^}]*line-height:\s*20px;[^}]*letter-spacing:\s*\.01em;[^}]*text-transform:\s*none;/s, `${label} must define the project-title treatment`);
