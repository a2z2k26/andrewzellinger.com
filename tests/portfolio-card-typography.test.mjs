...
function assertPortfolioTreatment(source, label) {
  ...
  assert.match(source, /\.heading-style-h2\.new\s*\{[^}]*font-family:\s*var\(--fonts--family-display\);[^}]*font-size:\s*24px;[^}]*line-height:\s*24px;[^}]*letter-spacing:\s*\.01em;[^}]*text-transform:\s*none;/s, `${label} must define the project-title treatment`);
