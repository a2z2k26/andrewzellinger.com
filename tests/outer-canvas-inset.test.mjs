...
test("Index image gaps follow the outer inset while Projects uses its own 24px card gap", async () => {
  const index = await readFile(new URL("../index.html", import.meta.url), "utf8");
  const details = await readFile(new URL("../src/detail-state.css", import.meta.url), "utf8");

  assert.match(index, /html\.index-route\s*\{[^}]*--index-image-gap:\s*var\(--structure--padding-desktop\);/s);
  assert.match(index, /@media screen and \(max-width: 991px\)\s*\{[^}]*html\.index-route\s*\{[^}]*--index-image-gap:\s*var\(--structure--padding-tablet\);/s);
