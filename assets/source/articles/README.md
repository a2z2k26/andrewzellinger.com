...
Andrew approved using these abstract artworks without requiring a relationship to article subject matter. The current article layout is text-only. These retained assignments and web copies support a future approved artwork treatment; they do not describe the current rendered layout.
...
- To add an article, assign one of the available files in the mapping, create its web copy, and move the file from Available to Assigned here. Rendering artwork in the listing, detail page or transitions requires an explicit design decision.
