...
The introduction's searching orb uses `thinking-orbs/engine` in the existing DOM-first modal. The package supplies the 64px searching preset, frame geometry and canvas painter. A 68px-high frame is projected into a 112×68px canvas by spreading only x positions; round dots remain round and the painted globe reads about 1.7:1. The bright scan uses 60% of the preset's previous sweep speed while the globe's base rotation keeps its original timing. The orb paints light dots on the dark panel, pauses when out of view or when the tab is hidden, and switches to the package's representative static frame for reduced motion. Its animation is independent of the panel entrance and the Projects carousel; modal close/page departure destroys its observer, listeners and frame loop. The orb is decorative to the already labelled dialog. History's portrait globe keeps its separate original SVG animation.
